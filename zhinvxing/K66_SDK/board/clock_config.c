...
/*
Bus_clock.outFreq       :176/3
Core_clock.outFreq      :176MHz
Flash_clock.outFreq     :176/7 MHz
FlexBus_clock.outFreq   :176/3 MHz
LPO_clock.outFreq       : 1 kHz
MCGFFCLK.outFreq        : 250 kHz
MCGIRCLK.outFreq        : 32.768 kHz
OSCERCLK.outFreq        : 16 MHz
OSCERCLK_UNDIV.outFreq  : 16 MHz
PLLFLLCLK.outFreq       : 176 MHz
System_clock.outFreq    : 176 MHz
*/
/*******************************************************************************
 * Variables for BOARD_BootClockRUN configuration
 ******************************************************************************/
const mcg_config_t SW_mcgConfig_BOARD_BootClockHSRUN = {
    .mcgMode         = kMCG_ModePEE,             //进入FBE模式                 
    .irclkEnableMode = kMCG_IrclkEnable | kMCG_IrclkEnableInStop,   //停止模式下使能"慢内参考时钟"
    .ircs            = kMCG_IrcSlow,             //选择慢内部参考时钟(参考MCG->C2的IRCS寄存器)                 
    .fcrdiv          = 0x0U,                     //慢内部参考时钟微调设置---0，不使用 外部时钟进来16M/1 = 16M                 
    .frdiv           = 0x1U,                     //FLL 外部参考分频，在MCG->C1的FRDIV中选择，FLL时钟分频因子，原本是12M/32 ，现在是16M/32 =250k               
    .drs             = kMCG_DrsLow,              //编码 0—低范围（复位默认）              
    .dmx32           = kMCG_Dmx32Default,        //RANGE=0，分频因子是 1              
    .oscsel          = kMCG_OscselOsc,           //配置失败时，选择Selects Oscillator（系统内部时钟输出）                
    .pll0Config =
        {
            .enableMode = MCG_PLL_DISABLE,       //MCGPLLCLK 没有激活，使能外部振荡器   
            .prdiv      = 0x0U,                  /* PLL Reference divider: divided by 1 */
            .vdiv       = 0x6U,                  /* VCO divider: multiplied by 30 */
        },
    .pllcs = kMCG_PllClkSelPll0, /* PLL0 output clock is selected */
};
const sim_clock_config_t SW_simConfig_BOARD_BootClockHSRUN = {
    .pllFllSel  = SIM_PLLFLLSEL_MCGPLLCLK_CLK,   //选择MCG时钟输出
    .pllFllDiv  = 0,                             //PLL选择1分频
    .pllFllFrac = 0,                             //写0，即0+1。
    .er32kSrc   = SIM_OSC32KSEL_RTC32KCLK_CLK, /* OSC32KSEL select: RTC32KCLK clock (32.768kHz) */
    .clkdiv1    = 0x2260000U,                  /* SIM_CLKDIV1 - OUTDIV1: /1, OUTDIV2: /3, OUTDIV3: /3, OUTDIV4: /7 */
};
const osc_config_t SW_oscConfig_BOARD_BootClockHSRUN = {
    .freq        = 16000000U,                     //晶振16M
    .capLoad     = (OSC_CAP0P),                   //选择0pf
    .workMode    = kOSC_ModeExt,          //OSC运行在低功耗模式下
    .oscerConfig = {
        .enableMode =
            kOSC_ErClkEnable,                     //选择使用外部参考时钟，在停止模式下禁用外部参考时钟
        .erclkDiv = 0,                            /* Divider for OSCERCLK: divided by 1 */
    }};
...
void board_clock_init(void)
{
    ...
    CLOCK_InitOsc0(&SW_oscConfig_BOARD_BootClockHSRUN);
    CLOCK_SetXtal0Freq(SW_oscConfig_BOARD_BootClockHSRUN.freq);

    CLOCK_SetInternalRefClkConfig(SW_mcgConfig_BOARD_BootClockHSRUN.irclkEnableMode, SW_mcgConfig_BOARD_BootClockHSRUN.ircs,
                                  SW_mcgConfig_BOARD_BootClockHSRUN.fcrdiv);

    CLOCK_CONFIG_SetFllExtRefDiv(SW_mcgConfig_BOARD_BootClockHSRUN.frdiv);

    CLOCK_BootToPeeMode(SW_mcgConfig_BOARD_BootClockHSRUN.oscsel, SW_mcgConfig_BOARD_BootClockHSRUN.pllcs,
                        &SW_mcgConfig_BOARD_BootClockHSRUN.pll0Config);
    ...
    CLOCK_SetSimConfig(&SW_simConfig_BOARD_BootClockHSRUN);
