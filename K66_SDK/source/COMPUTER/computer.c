...
#define CMD_IMG     1
    ...
    UART_WRETE_BUFF(cmdf, sizeof(cmdf));    //先发送命令

    UART_WRETE_BUFF((uint8_t *)imgaddr, imgsize); //再发送图像

    UART_WRETE_BUFF(cmdr, sizeof(cmdr));    //先发送命令
}
...
#define CMD_CCD     2
    ...
    UART_WRETE_BUFF((uint8_t *)ccdaddr, ccdsize); //再发送图像

    UART_WRETE_BUFF(cmdr, sizeof(cmdr));    //再发送命令
}
...
#define CMD_WARE     3
    ...
    UART_WRETE_BUFF(cmdf, sizeof(cmdf));    //先发送前命令
    UART_WRETE_BUFF((uint8_t *)wareaddr, waresize);    //发送数据
    UART_WRETE_BUFF(cmdr, sizeof(cmdr));    //发送后命令
}
